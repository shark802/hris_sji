		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
                    ...
                    //Legal
                      $holsql = "SELECT * FROM holidayschedules where holiday_type = 'Legal'";               
                      $holquery = $conn->query($holsql);
                      $holrow = $holquery->fetch_array();
                      $legal = $holrow['id'];
                      $datelegal = $holrow['date']; 
                    
                    //Special
                      $holsql1 = "SELECT * FROM holidayschedules where date = '$date_now'";               
                      $holquery1 = $conn->query($holsql1);
                      $holrow1 = $holquery1->fetch_array();
                      $special = $holrow1['id'];
                      $datespecial = $holrow1['date']; 
